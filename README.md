...
- **모델**: `--model`은 콤마 목록이고, 각 모델은 `src/llm/openai.ts`의 structured output 백엔드로 Jev와 같은 15개 질문을 받습니다. OpenAI SDK가 `OPENAI_BASE_URL`을 읽으므로 OpenAI 호환 엔드포인트를 코드 변경 없이 쓸 수 있습니다(json_schema 응답 형식을 지원하는 모델이어야 합니다). 다른 회사 모델을 넣을 때는 그 회사의 OpenAI 호환 엔드포인트를 직접 쓰는 편이 무엇이 실행됐는지 분명합니다(Gemini: `https://generativelanguage.googleapis.com/v1beta/openai/`, Claude: `https://api.anthropic.com/v1/`). OpenRouter 같은 라우터는 한 키로 여러 회사 모델을 부를 수 있지만 기본 설정이 가격 기준으로 제공자를 고르고 제공자마다 양자화가 다를 수 있어, 벤치마크에 쓰려면 요청 본문에 `provider: { order: [...], allow_fallbacks: false }`를 넣어 고정해야 합니다(이 코드에는 없습니다). 어느 쪽이든 응답이 알려준 실제 모델은 `decisions.jsonl`의 `served`에 남고 bench.md에 표시됩니다. 이미 답한 (모델, 논문) 쌍은 건너뛰므로 중간에 실패해도 다시 돌리면 이어지고, 결과는 누적됩니다.
